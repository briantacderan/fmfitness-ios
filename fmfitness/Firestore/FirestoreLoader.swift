...
final class FirestoreLoader: ObservableObject {
    ...
    private lazy var baseUrlString: String? = { "https://firestore.googleapis.com/v1/projects/fm-fitness/databases/(default)/documents/profile/\(self.email)"
    }()
    
    private lazy var apptUrlString: String? = { "https://firestore.googleapis.com/v1/projects/fm-fitness/databases/(default)/documents/appointment/\(self.email)"
    }()
    ...
    private func sessionWithFreshToken(completion: @escaping (Result<URLSession, Error>) -> Void) {
        ...
        authentication?.do { [weak self] auth, error in
            guard let token = auth?.accessToken else {
                completion(.failure(.couldNotCreateURLSession(error)))
                print(self?.components?.url ?? "No URL available")
                return
            }
            let configuration = URLSessionConfiguration.default
            configuration.httpAdditionalHeaders = [
